...
def run(config):
    ...
    model = AttentionSAC.init_from_save('model.pt', load_critic=True)
    model.critic_optimizer = Adam(model.critic.parameters(), lr=config['q_lr'], weight_decay=1e-3)
    for agent in model.agents:
        agent.policy_optimizer = Adam(agent.policy.parameters(), lr=config['pi_lr'])
    ...
    for ep_i in range(0, config["n_episodes"], config["n_rollout_threads"]):
        ...
        for step in range(config["episode_length"]):
            ...
            if (len(replay_buffer) >= int(1e6) and
